...
export default function edit( { attributes, setAttributes, clientId } ) {
	...
	const PLAYLIST_TEMPLATE = [
		[
			'core/image',
			{ align: 'center', style: { border: { radius: '5px' } } },
		],
		[
			'core/heading',
			{ placeholder: 'Playlist title', textAlign: 'center', level: 3 },
		],
	];
	...
	const setNowPlaying = ( item ) => {
		...
		document.getElementById( pID ).querySelector( '.playing' ).textContent =
			num + ' - ' + title;
	};
	...
	const setVolumeSlider = () => {
		document.getElementById( pID ).querySelector( '.volume-slider' ).value =
			document.getElementById( pID ).querySelector( '.player' ).volume;
	};
	...
	const clickMuteButton = () => {
		...
		document
			.getElementById( pID )
			.querySelector( '.volume-slider' ).style.backgroundSize = '0% 100%';
	};
	...
	const clickUnMuteButton = () => {
		...
		document
			.getElementById( pID )
			.querySelector( '.volume-slider' ).style.backgroundSize =
			'100% 100%';
	};
	...
	const loadMetaData = ( event ) => {
		...
		if ( elems ) {
			document
				.getElementById( pID )
				.querySelector( '.total-time' ).innerHTML = convertElapsedTime(
				event.target.duration
			);
			document
				.getElementById( pID )
				.querySelector( '.current-time' ).innerHTML =
				convertElapsedTime( event.target.currentTime );
			document
				.getElementById( pID )
				.querySelector( '.time-sep' ).innerHTML = ' / ';
			const seekSlider = document
				.getElementById( pID )
	...
	const timeUpdate = ( event ) => {
		document
			.getElementById( pID )
			.querySelector( '.current-time' ).innerHTML = convertElapsedTime(
	...
	const onChangeSlider = ( event ) => {
		document.getElementById( pID ).querySelector( '.player' ).currentTime =
			event.target.value;
	};

	const changeVolume = ( event ) => {
		const mp3Player = document
			.getElementById( pID )
	...
	const onClickTrack = ( event ) => {
		...
		document.getElementById( pID ).querySelector( '.player' ).src =
			event.target.href;
	...
	const nowPlaying = tracks.map( ( track, index ) => {
		if ( index === 0 ) {
			return <p key={ index } className="playing">01 - { track.title }</p>;
		}
	} );
	...
	const trackItems = tracks.map( ( track, index ) => {
		...
		return (
			<li key={ track.trackid } className={ classActive }>
				{ track.url && (
					<a
						href={ track.url }
						className="item"
					...
					>
						...
						<span className="right">{ track.duration }</span>
					</a>
				) }
				<MediaUploadCheck>
					<MediaUpload
				...
				<div id={ pID }>
					...
					<div className="volume-container">
						<div className="volume-show">
							<label htmlFor="volume-slider" hidden>
								Volume
							</label>
							<input
							...
							>
								...
								<span id="vol-muted-label" hidden>
									Unmute volume
								</span>
							</button>
						</div>
						...
						>
							...
							<span id="vol-repeat-label" hidden>
								Repeat all
							</span>
						</button>
					</div>
					...
					<div className="seek-container left">
						<label htmlFor="seek-slider" hidden>
							Seek
						</label>
						<input
